...
class IFU()(implicit val p: Parameters) extends MyModule {
    ...
    btbs.zipWithIndex.foreach{ case(btb, i) => 
        btb.io.write.valid := backendRedirect && redirectChosenOH(i) || s1_redirectJumpOH(i) && !s1_btbInfo(i).hit
        btb.io.write.bits.jump := Mux(backendRedirect, false.B, s1_isJumpVec(i))
        btb.io.write.bits.pc := Mux(backendRedirect, s0_redirectInfo.pc, s1_instPC + (i * 4).U )
        btb.io.write.bits.target := Mux(backendRedirect, s0_redirectInfo.targetAddr, s1_targetAddrVec(i)) 
    }

    phts.zipWithIndex.foreach{ case(pht, i) => 
        pht.io.update.valid := backendRedirect && redirectChosenOH(i) || s1_redirectJumpOH(i) && !s1_brTaken(i)
        pht.io.update.bits.brTaken := Mux(backendRedirect, s0_redirectInfo.brTaken, true.B)
