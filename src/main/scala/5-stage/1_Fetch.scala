...
class Fetch_1()(implicit val p: Parameters) extends MyModule{
    ...
    val stall = io.ctrl.stall || !io.in.start || !icache.io.read.req.ready || RegNext(!instValid) || !RegNext(io.in.start)
    // val stall = io.ctrl.stall || !io.in.start || !icache.io.read.req.ready || !RegNext(io.in.start)
    val flush = io.ctrl.flush
    ...
    icache.io.read.req.bits.addr := pc
