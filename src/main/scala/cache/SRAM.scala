...
class BankRam2P(width: Int = 64, depth: Int = 16, maskSegments: Int = 8)extends BaseSram {
    val io = IO(new Bundle{
        val r = new Bundle{
            val en = Input(Bool())
            val addr = Input(UInt(log2Ceil(depth).W))
    ...
    when(io.w.en) {
        ...
        // printf(cf"[BankRam2P] write addr: ${io.w.addr} data: ${wdata} \n")
    }
...
class BankRam2P_1(width: Int = 64, depth: Int = 16, maskSegments: Int = 8, combRead: Boolean = true)extends BaseSram {
    val io = IO(new Bundle{
        val r = new Bundle{
            val en = Input(Bool())
            val addr = Input(UInt(log2Ceil(depth).W))
    ...
    // read data will be stable until next read enable
    if(combRead == true) {
        io.r.data := Cat(rdata.reverse).asUInt
    } else {
        io.r.data := RegEnable(Cat(rdata.reverse), 0.U(width.W), io.r.en)
    }
...
// class SRAMTemplate_1[T <: Data](gen: T, nrSet: Int = 1024, nrWay: Int = 4, singlePort: Boolean = true) extends Module {
//     val io = IO(new Bundle{
//         val r = new Bundle{
//             val en = Input(Bool())
//             val addr = Input(UInt(log2Ceil(nrSet).W))
//             val data = Output(UInt((gen.getWidth * nrWay).W))
//         }
//         val w = new Bundle{
//             val en = Input(Bool())
//             val addr = Input(UInt(log2Ceil(nrSet).W))
//             val data = Input(UInt((gen.getWidth * nrWay).W))
//             val mask = if(nrWay > 1) Some(Input(UInt(nrWay.W))) else None
//         }
//     })

//     val sram = SRAM(gen.getWidth * nrWay, nrSet, nrWay, singlePort)
