...
class RefillReqBundle_1()(implicit val p: Parameters) extends MyBundle {
    val addr = UInt(xlen.W)
    val chosenWay = UInt(dcacheWays.W)
}

class RefillRespBundle_1()(implicit val p: Parameters) extends MyBundle {
    val data = UInt((dcacheBlockBytes*8).W)
}

class RefillPipeIO_1()(implicit val p: Parameters) extends MyBundle {
...
class RefillPipe_1()(implicit val p: Parameters) extends MyModule {
    import RefillPipe_1.State
    import RefillPipe_1.State._ 

    val io = IO(new RefillPipeIO_1)
    io <> DontCare
    ...
    io.dirWrite.req.bits.addr := reqReg.addr // req.addr
    val meta = Wire(new DCacheMeta)
    meta.valid := true.B 
    meta.dirty := false.B
    io.dirWrite.req.bits.meta := meta.asUInt
    io.dirWrite.req.bits.way := reqReg.chosenWay //req.chosenWay

    // writeback databank
    io.dataWrite.req.valid := refillSafe
    io.dataWrite.req.bits.blockSelOH := beatOH
    io.dataWrite.req.bits.way := reqReg.chosenWay //req.chosenWay
    // io.dataWrite.req.bits.mask := Fill(dcacheBlockBytes, 1.U)
    io.dataWrite.req.bits.set := addrToDCacheSet(reqReg.addr) //addrToDCacheSet(req.addr)
